...
import { MIGRATIONS, GUEST_USER_ID } from "./migrations";
...
export async function initializeDatabase(): Promise<void> {
  ...
  `);
    ...
    const layerSeedRow = await db.getFirstAsync<{ count: number }>(
        `SELECT COUNT(*) as count FROM schedule_layers WHERE user_id = ?`,
        [GUEST_USER_ID],
    );

    if (!layerSeedRow || layerSeedRow.count === 0) {
        const defaults = [
        ...
        for (const layer of defaults) {
         ...
         VALUES (?, ?, ?, ?, 1)`,
                [generateId(), GUEST_USER_ID, layer.layerName, layer.colorCode],
            );
        }
    }
}
